...
from utils.loss_utils import CombinedLoss
...
class PromptIRModel(pl.LightningModule):
    ...
    def test_step(self, batch, batch_idx):
        ...
        batch_size, c, h, w = degrad_patch.shape
        tta_transforms = [
            lambda x: x,
        ...
        for transform in tta_transforms:
            ...
            transformed_output = self.net(transformed_input)

            # Restore
            if transform == tta_transforms[1]:
                transformed_output = torch.flip(transformed_output, [3])
            elif transform == tta_transforms[2]:
                transformed_output = torch.flip(transformed_output, [2])
            elif transform == tta_transforms[3]:
                transformed_output = torch.flip(
                    torch.flip(transformed_output, [2]), [3]
                )
